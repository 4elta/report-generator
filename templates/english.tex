...
Vulnerabilities with severity scores between \VAR{ severity_range.min } and \VAR{ severity_range.max } (on a scale from 0--10) have been found (see Table~\ref{tab:vulnerabilities}).
  ...
  \begin{table}[h]
    ...
    \begin{tabular}{ll}
      ...
      \BLOCK{ for issue in issues|sort(attribute='severity.number', reverse=true) if issue.group.order == group.order }
        \hyperref[\VAR{ issue.label }]{\VAR{ issue.title }} & \smallseveritygauge[0.2]{\VAR{ issue.severity.number }} \VAR{ issue.severity.number } \\
      \BLOCK{ endfor }
    \end{tabular}
  \end{table}
\BLOCK{ endfor }
