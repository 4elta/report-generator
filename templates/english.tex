...
\BLOCK{ for group in groups }
  ...
  \BLOCK{ for issue in issues|sort(attribute='severity.number', reverse=true) if issue.group.order == group.order }
    ...
    \BLOCK{ for evidence in issue.evidence }
      \paragraph{\VAR{ evidence.id }}

      \VAR{ evidence.description|markdown2latex }
    \BLOCK{ endfor }

    \subsubsection{Affected Assets}

    \begin{itemize}
      \BLOCK{ for asset in issue.affected_assets }
        \item \VAR{ asset }
      \BLOCK{ endfor }
    \end{itemize}
    ...
    \begin{center}
      \severitygauge{\VAR{ issue.severity.number }}{\VAR{ issue.severity.number } (\VAR{ issue.severity.class|lower })}
    ...
    \BLOCK{ for image in issue.images }
      \begin{figure}[p]
        ...
        \label{IMG:\VAR{ issue.label }-\VAR{ image.file }}
      \end{figure}
    \BLOCK{ endfor }
